...
export const useProfileForm = () => {
  ...
  const isFormValid = () => {
    ...
    const passwordConfirmValid = validatePasswordConfirm(formData.passwordConfirm, formData.password);
    const nameValid = !validateName(formData.name);
    const phoneValid = !validatePhone(formData.phone);
    const genderValid = !validateGender(formData.gender);

    return !passwordValid.error && !passwordConfirmValid.error && nameValid && phoneValid && genderValid;
  };
